...
public class Score2 {
	...
	public Score2(String name, int mathScore, int korScore, int engScore) {
		this.name = name;
	...
	public static void main(String[] args) {
		...
		List<Score2> list = new ArrayList<Score2>();
		boolean run = true;
	...
	public Score2 getscore(Score2 score) {     // 학생, 점수 입력
		Scanner scanner = new Scanner(System.in);
		...
		score.setKorScore(korScore);
		System.out.print("영어: ");
		int engScore = Integer.parseInt(scanner.nextLine());
		score.setEngScore(engScore);
		System.out.print("수학: ");
	...
	public void getAvg(List<Score2> list) {    // 평균 산출
		int avg = (list.stream().mapToInt(t -> t.getSum()/3).sum()/list.size());
		System.out.println("전체 평균: " + avg);
	}
	
	public void getPerAvg(List<Score2> list) {    // 과목별 평균 산출
	...
	public void getMaxScore(List<Score2> list) {  // 최대값 산출
		Scanner scanner = new Scanner(System.in);
